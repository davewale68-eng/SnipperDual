...
class MemoryStore {
  constructor() {
    ...
    // along with their persistent state below. fourSilHitAverageEngine.js
    // (kept) is now stateless -- it derives its one surviving stat
    // directly from entryHitCounter each cycle, so it no longer needs a
    // slot here either.
    ...
    // 3-Ball Color Hit Counter's persistent watch state -- the OTHER of
    // the project's two 3-ball color forecasts (threeBallColorEngine.js's
    // own trackedColor, the original/main 3-Ball Color Hit Intelligence
    // card) gets its own continuous hit/miss scoring log here, direct
    // structural clone of threeBallNextEventHitCounter above (no rank
    // watch -- this engine has a single tracked-color pick, not a
    // 3-color leaderboard). See threeBallColorHitCounter.js's header.
    this.threeBallColorHitCounter = {
      schemaVersion: 1,
      lastProcessedDrawId: null,
      totalCalls: 0,
      trackedColorPick: { hits: 0, misses: 0, pending: null },
      updatedAt: null
    };
    ...
    this.tierOverduePrecursorHitCounter = {
      schemaVersion: 2, // v2: MISS-LOCK (missLockTieDrawId) + batch-walk fix -- see tierOverduePrecursorHitCounter.js's header
      lastProcessedDrawId: null,
      totalCalls: 0,
    ...
    // Zero Color Hit Counter's persistent watch state -- the Zero Color
    // Engine's own OVERDUE/WATCH CLOSELY-call scoring log, direct
    // structural port of tierOverduePrecursorHitCounter.js above but
    // watching for a landed zero event (not a landed tie) over the same
    // 8-draw window. See zeroColorHitCounter.js's header.
    this.zeroColorHitCounter = {
      schemaVersion: 2,
      lastProcessedDrawId: null,
      totalCalls: 0,
    ...
    // 4-Ball Color Next Event Hit Counter's persistent watch state --
    // scores fourBallColorNextEventEngine.js's own predictedNextColor
    // continuously (every cycle has a dueColor once there's any
    // sample, no gate), direct structural clone of
    // threeBallNextEventHitCounter/threeBallColorHitCounter above. See
    // fourBallColorNextEventHitCounter.js's header.
    this.fourBallColorNextEventHitCounter = {
      schemaVersion: 2, // v2: dual-pick rewire (pick1Slot/pick2Slot + mirrorLockDrawId) -- see fourBallColorNextEventHitCounter.js's header
      lastProcessedDrawId: null,
      totalCalls: 0,
      mirrorLockDrawId: null,
      pick1Slot: { hits: 0, misses: 0, pending: null, predictions: [] },
      pick2Slot: { hits: 0, misses: 0, pending: null, predictions: [] },
      predictions: [], // combined, labeled log across both pick slots
      updatedAt: null
    };
    // Window Section Tracker's persistent tally -- which section of the
    // shared 8-draw window (~4 zone vs overdue +1..+4) the Zero Color and
    // 4-Ball Next Event HITs landed in. Bare placeholder; the tracker
    // fills its own fields in place. See windowSectionTracker.js.
    this.windowSectionTracker = { schemaVersion: 1, updatedAt: null };
    this.ingestStats = {
  ...
  resetStore() {
    ...
    // NOTE: fourBallEnterCallLog, paperTrader (4SIL Paper Trader), and
    // fourSilNextEventIntel were removed per operator direction -- see
    // the constructor's equivalent NOTE for details. fourSilHitAverage
    // is now stateless (derived fresh from entryHitCounter every cycle
    // in council.js), so there's nothing to wipe for it either.
    ...
    // reasoning as tieNextEvent above). Config
    // (ladder, startingCapital, payoutMultiplier) is preserved across the
    // reset, same convention as every other paper trader's reset line.
    this.tiePaperTrader = require('../engines/tiePaperTraderEngine').freshTiePaperTraderState(
    ...
    // Same reasoning, 3-Ball Color Hit Counter counterpart (see the
    // constructor's comment above this same field).
    this.threeBallColorHitCounter = {
      schemaVersion: 1,
      lastProcessedDrawId: null,
      totalCalls: 0,
    ...
    // Same reasoning, Zero Color Engine counterpart.
    this.zeroColorHitCounter = {
      schemaVersion: 2,
      lastProcessedDrawId: null,
      totalCalls: 0,
    ...
    // Same reasoning, 4-Ball Color Next Event Engine counterpart -- its
    // pending watch is cross-cycle state tied to specific draw IDs that
    // a reset wipes out from under it. See
    // fourBallColorNextEventHitCounter.js's header.
    this.fourBallColorNextEventHitCounter = {
      schemaVersion: 2, // v2: dual-pick rewire (pick1Slot/pick2Slot + mirrorLockDrawId) -- see fourBallColorNextEventHitCounter.js's header
      lastProcessedDrawId: null,
    ...
    // Same reasoning as the two hit counters above: the tracker's tally is
    // keyed to those counters' resolved logs, which this reset just wiped,
    // so it resets with them or its totals would exceed theirs.
    this.windowSectionTracker = { schemaVersion: 1, updatedAt: null };
    this.ingestStats = {
      totalDrawsIngested: 0,
      acceptedDraws: 0,
