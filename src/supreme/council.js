...
const { evaluateWindowSectionTracker } = require('../engines/windowSectionTracker');
...
function runSupremeCouncil() {
  ...
  // 4-Ball Color Next Event Hit Counter -- see fourBallColorNextEventHitCounter.js's
  // header for full mechanics. Moved to run AFTER fourSIL (previously sat
  // right after fourBallColorNextEvent above, before fourSIL existed in
  // this cycle's scope) and now gated on fourSIL.badge.enterNow, same
  // discipline as entryHitCounter.js just below. See fourBallColorNextEventHitCounter.js
  // for why this changed: the hero card's dual pick is now itself gated
  // on 4SIL's ENTER signal, so a counter that opened a fresh watch every
  // single cycle regardless of that gate was logging calls the card was
  // never actually making -- inflating totalCalls well past what the
  // card's real ENTER-gated call volume should be.
  const fourBallColorNextEventHitCounter = evaluateFourBallColorNextEventHitCounter(historicalDraws, fourBallColorNextEvent, store.fourBallColorNextEventHitCounter);

  // Window Section Tracker -- read-only observer over the Zero Color and
  // 4-Ball Next Event hit counters' own resolved logs. Same 8-draw window,
  // unchanged; records WHICH part of it (~4 countdown zone vs overdue
  // +1..+4) each HIT landed in. Must run after BOTH counters above. See
  // windowSectionTracker.js's header. Touches neither counter nor engine.
  const windowSectionTracker = evaluateWindowSectionTracker(zeroColorHitCounter, fourBallColorNextEventHitCounter, store.windowSectionTracker);
  ...
  const snapshot = {
    ...
    // Window Section Tracker -- see windowSectionTracker.js. Per-engine
    // count of which section of the 8-draw window (~4 zone vs overdue
    // +1..+4) the Zero Color and 4-Ball Next Event HITs landed in.
    windowSectionTracker,
