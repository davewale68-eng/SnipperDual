...
const { advanceFourBallEnterCallLog } = require('../engines/fourBallEnterCallLog');
const { evaluateFourSilHitAverage } = require('../engines/fourSilHitAverageEngine');
const { advancePaperTrader } = require('../engines/paperTraderEngine');
...
const { evaluateFourSilNextEventIntel } = require('../engines/fourSilNextEventIntelEngine');
...
function runSupremeCouncil() {
  ...
  // 4SIL ENTER Call — 4-Ball Event Density Log. Tracks each ENTER call
  // from its opening draw ID to close, counting every 4-ball event of ANY
  // color that lands inside the window. Color-agnostic: RED, BLUE and GREEN
  // are treated identically — only the presence of a 4-ball event matters.
  // Runs after entryHitCounter so both share the same ordering guarantee:
  // resolve against the latest landed draw before opening new watches.
  const fourBallEnterCallLog = advanceFourBallEnterCallLog(historicalDraws, fourSIL, store.fourBallEnterCallLog);

  // 4SIL Hit Average Engine — runs AFTER entryHitCounter and
  // fourBallEnterCallLog so it can read both results for the same cycle.
  // Consumes closed-call data to compute avg per-color hits, dual hits,
  // and total 4-ball events per ENTER call. Maintains its own 7-day
  // auto-pruned hit log (store.fourSilHitAverage).
  const fourSilHitAverage = evaluateFourSilHitAverage(
    entryHitCounter,
    fourBallEnterCallLog,
    store.fourSilHitAverage
  );

  // 4SIL Paper Trader Engine -- v1.1 blueprint. Consumes ONLY 4SIL's own
  // signal (never fourBallP's picks/colors) and this cycle's draw stream
  // to paper-trade the fixed RED+BLUE+GREEN ladder strategy. Runs after
  // 4SIL has fully computed for this cycle, same ordering guarantee as
  // entryHitCounter/fourBallEnterCallLog above. Purely an evaluation
  // consumer -- its output is never read back into 4SIL or any gate/weight
  // (see paperTraderEngine.js header, ABSOLUTE SEPARATION RULE).
  const paperTrader = advancePaperTrader(historicalDraws, fourSIL, store.paperTrader);

  // 4SIL Next Event Intelligence Engine (4SIL-NEI) -- downstream timing
  // analyst, NOT another 4SIL gate. Fuses fourBallEnterCallLog's signal
  // latency/event-spacing history with paperTrader's real HIT/MISS
  // spacing into a color-agnostic "when should the next 4-ball event
  // land" forecast. Runs LAST among the 4SIL-family engines, once both
  // fourBallEnterCallLog and paperTrader have fully advanced for this
  // cycle, since it reads both. Purely observational -- never votes and
  // never feeds back into 4SIL or any gate/weight (see that engine's
  // header, ABSOLUTE SEPARATION RULE).
  const fourSilNextEventIntel = evaluateFourSilNextEventIntel(
    fourSIL,
    fourBallEnterCallLog,
    paperTrader,
    store.fourSilNextEventIntel
  );
  ...
  const snapshot = {
    ...
    // fourBallEnterCallLog.js. Tracks each ENTER call from openDrawId to
    // closeDrawId, recording every 4-ball event of ANY color that landed
    // inside the window (color-agnostic: only the presence of a 4-ball
    // event counts, not which color). Purely observational — never votes
    // and never feeds 4SIL or any other engine.
    fourBallEnterCallLog,
    // 4SIL Hit Average Engine -- see fourSilHitAverageEngine.js.
    // Avg per-color hits, avg dual hits, and avg total 4-ball events per
    // 4SIL ENTER call. 7-day auto-pruned hit log maintained separately.
    fourSilHitAverage,
    paperTrader,
    // 4SIL Next Event Intelligence Engine (4SIL-NEI) -- see
    // fourSilNextEventIntelEngine.js. Color-agnostic forecast of when the
    // next 4-ball event should land, fused from fourBallEnterCallLog +
    // paperTrader timing profiles. Purely observational -- never votes
    // and never feeds 4SIL or any other engine.
    fourSilNextEventIntel,
