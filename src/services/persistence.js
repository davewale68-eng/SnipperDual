...
function saveSnapshot(store) {
  ...
  const state = {
    ...
    // Tier Engine Overdue/Precursor Hit Counter -- scores the Tie
    // Engine's own OVERDUE/PRECURSOR calls against whether a 3-ball tie
    // actually lands. Was missing from persistence entirely; added
    // alongside the MISS-LOCK + batch-walk fix so missLockTieDrawId and
    // any pending watch survive a restart instead of silently resetting.
    // See tierOverduePrecursorHitCounter.js's header.
    tierOverduePrecursorHitCounter: store.tierOverduePrecursorHitCounter,
    ...
    // Window Section Tracker -- cumulative per-section HIT tally over the
    // two hit counters' logs above. The counters only keep their last 20
    // resolved entries, so this tally cannot be recomputed from them and
    // must persist. See windowSectionTracker.js's header.
    windowSectionTracker: store.windowSectionTracker
  };
...
function loadSnapshot(store) {
  ...
  try {
    ...
    // zeroColorHitCounter.js's header.
    // v2 REWIRE: added predictions log (rolling HIT/MISS history) and
    // draw-accurate miss detection. v1 snapshots (schemaVersion 1 or
    // missing) are migrated: hits/misses carry over, predictions log
    // starts empty (no history to backfill). See zeroColorHitCounter.js.
    if (data.zeroColorHitCounter && typeof data.zeroColorHitCounter === 'object') {
      ...
      const storedVersion = Number(zc.schemaVersion) || 1;
      store.zeroColorHitCounter = {
        ...
        predictions: storedVersion >= 2 && Array.isArray(zc.predictions) ? zc.predictions : [],
        updatedAt: zc.updatedAt || null
      };
    }

    // Backward-compatible restore for the Tier Engine Overdue/Precursor
    // Hit Counter. This field was never wired into persistence before,
    // so older snapshots simply have no field and retain the store's
    // defaults (starts fresh, no history lost since there was never any
    // to restore). v2: added missLockTieDrawId as part of the MISS-LOCK
    // fix. See tierOverduePrecursorHitCounter.js's header.
    if (data.tierOverduePrecursorHitCounter && typeof data.tierOverduePrecursorHitCounter === 'object') {
      const tc = data.tierOverduePrecursorHitCounter;
      const restoreTierPick = p => ({
        hits: Number(p && p.hits) || 0,
        misses: Number(p && p.misses) || 0,
        pending: p && p.pending && typeof p.pending === 'object' ? p.pending : null
      });
      store.tierOverduePrecursorHitCounter = {
        schemaVersion: 2,
        lastProcessedDrawId: tc.lastProcessedDrawId ?? null,
        totalCalls: Number(tc.totalCalls) || 0,
        tierCall: restoreTierPick(tc.tierCall),
        missLockTieDrawId: tc.missLockTieDrawId ?? null,
    ...
    // store's defaults. DUAL-PICK REWIRE (schemaVersion 2): the single
    // predictedColorPick slot became pick1Slot/pick2Slot plus a combined
    // predictions log and mirrorLockDrawId -- see
    // fourBallColorNextEventHitCounter.js's header. A pre-rewire
    // (schemaVersion 1) snapshot's predictedColorPick is carried over
    // into pick1Slot only (it was watching the same "overdue color" the
    // new pick1Slot watches by default), and pick2Slot starts fresh --
    // this loses no history, it just means pick2Slot's own log starts
    // measuring from whenever the rewire deployed.
    if (data.fourBallColorNextEventHitCounter && typeof data.fourBallColorNextEventHitCounter === 'object') {
      ...
      const restoreFcSlot = p => ({
        hits: Number(p && p.hits) || 0,
        misses: Number(p && p.misses) || 0,
        pending: p && p.pending && typeof p.pending === 'object' ? p.pending : null,
        predictions: Array.isArray(p && p.predictions) ? p.predictions : []
      });
      const isPreRewire = !fc.pick1Slot && fc.predictedColorPick;
      store.fourBallColorNextEventHitCounter = {
        schemaVersion: Number(fc.schemaVersion) || (isPreRewire ? 1 : 2),
        lastProcessedDrawId: fc.lastProcessedDrawId ?? null,
        totalCalls: Number(fc.totalCalls) || 0,
        mirrorLockDrawId: fc.mirrorLockDrawId ?? null,
        pick1Slot: restoreFcSlot(isPreRewire ? fc.predictedColorPick : fc.pick1Slot),
        pick2Slot: restoreFcSlot(fc.pick2Slot),
        predictions: Array.isArray(fc.predictions) ? fc.predictions : [],
        updatedAt: fc.updatedAt || null
      };
    }

    // Backward-compatible restore for the Window Section Tracker. Older
    // snapshots simply have no field and retain the store's placeholder.
    // The tracker normalizes/fills its own fields in place on its next
    // evaluation, so only a plain-object check is needed here. See
    // windowSectionTracker.js.
    if (data.windowSectionTracker && typeof data.windowSectionTracker === 'object'
        && !Array.isArray(data.windowSectionTracker)) {
      store.windowSectionTracker = data.windowSectionTracker;
    }
    return true;
  } catch (err) {
