...
function saveSnapshot(store) {
  ...
  const state = {
    ...
    // 4SIL ENTER Call — 4-Ball Event Density Log. Cannot be recomputed
    // from draw history alone (a draw record doesn't store whether 4SIL
    // was calling ENTER when it landed). Must survive restarts so the
    // full call history and open-call state are preserved. Same reasoning
    // as entryHitCounter directly above.
    fourBallEnterCallLog: store.fourBallEnterCallLog,
    // 4SIL Hit Average Engine -- per-call hit log with 7-day auto-pruning.
    // Must be saved so accumulated history survives restarts.
    fourSilHitAverage: store.fourSilHitAverage,
    // 4SIL Paper Trader Engine's full ledger (see paperTraderEngine.js).
    // Must survive restarts: lastProcessedDrawId, the open cycle/window,
    // capital, and every closed trade/window/cycle/reset are historical
    // facts, not values that can be recomputed from historicalDraws once
    // the underlying draw ages out of the rolling window.
    paperTrader: store.paperTrader,
    // 4SIL Next Event Intelligence Engine's persistent state (see
    // fourSilNextEventIntelEngine.js). Must survive restarts: the frozen
    // activePrediction and the evaluations/calibration log are historical
    // facts tied to specific callIds, not values recomputable from
    // historicalDraws alone.
    fourSilNextEventIntel: store.fourSilNextEventIntel,
...
function loadSnapshot(store) {
  ...
  try {
    ...
    // Backward-compatible restore for the 4SIL ENTER Call Density Log.
    // Older snapshots have no field — store defaults (fresh empty state)
    // are retained. The open call's eventDrawIds array is restored
    // defensively as an empty array if absent (schema v1 always writes it
    // but older in-flight snapshots may predate the field).
    if (data.fourBallEnterCallLog && typeof data.fourBallEnterCallLog === 'object') {
      const { freshFourBallEnterCallLogState } = require('../engines/fourBallEnterCallLog');
      const d = data.fourBallEnterCallLog;
      const restoreOpenCall = oc => {
        if (!oc || typeof oc !== 'object') return null;
        return {
          callId: oc.callId ?? null,
          openDrawId: oc.openDrawId ?? null,
          openAt: oc.openAt ?? null,
          closeDrawId: oc.closeDrawId ?? null,
          closeAt: oc.closeAt ?? null,
          closeReason: oc.closeReason ?? null,
          draws: Number(oc.draws) || 0,
          fourBallEvents: Number(oc.fourBallEvents) || 0,
          eventDrawIds: Array.isArray(oc.eventDrawIds) ? oc.eventDrawIds : []
        };
      };
      const restoreClosedCall = cc => ({
        callId: cc.callId ?? null,
        openDrawId: cc.openDrawId ?? null,
        openAt: cc.openAt ?? null,
        closeDrawId: cc.closeDrawId ?? null,
        closeAt: cc.closeAt ?? null,
        closeReason: cc.closeReason ?? null,
        draws: Number(cc.draws) || 0,
        fourBallEvents: Number(cc.fourBallEvents) || 0,
        eventDrawIds: Array.isArray(cc.eventDrawIds) ? cc.eventDrawIds : []
      });
      store.fourBallEnterCallLog = {
        schemaVersion: Number(d.schemaVersion) || 1,
        lastProcessedDrawId: d.lastProcessedDrawId ?? null,
        openCall: restoreOpenCall(d.openCall),
        closedCalls: Array.isArray(d.closedCalls) ? d.closedCalls.map(restoreClosedCall) : [],
        totalCalls: Number(d.totalCalls) || 0,
        updatedAt: d.updatedAt || null
      };
    }

    // Backward-compatible restore for the 4SIL Hit Average Engine's
    // per-call hit log. Older snapshots have no field and retain the
    // store's fresh defaults. The 7-day pruning is re-applied on restore
    // so stale entries from a crashed/paused server are trimmed immediately.
    if (data.fourSilHitAverage && typeof data.fourSilHitAverage === 'object') {
      const { freshFourSilHitAverageState, pruneOldEntries } = require('../engines/fourSilHitAverageEngine');
      const ha = data.fourSilHitAverage;
      const restoreHitEntry = e => ({
        callId:        e.callId        != null ? e.callId        : null,
        openDrawId:    e.openDrawId    != null ? e.openDrawId    : null,
        closeDrawId:   e.closeDrawId   != null ? e.closeDrawId   : null,
        firstColor:    e.firstColor    || null,
        secondColor:   e.secondColor   || null,
        firstPickHit:  Boolean(e.firstPickHit),
        secondPickHit: Boolean(e.secondPickHit),
        dualHit:       Boolean(e.dualHit),
        fourBallEvents: Number(e.fourBallEvents) || 0,
        closeReason:   e.closeReason   || null,
        recordedAt:    e.recordedAt    || null
      });
      const hitLog = Array.isArray(ha.hitLog) ? ha.hitLog.map(restoreHitEntry) : [];
      // Re-apply 7-day prune immediately on restore
      pruneOldEntries(hitLog);
      store.fourSilHitAverage = {
        schemaVersion:        Number(ha.schemaVersion) || 1,
        lastProcessedCallId:  ha.lastProcessedCallId != null ? ha.lastProcessedCallId : null,
        hitLog,
        updatedAt:            ha.updatedAt || null
      };
    }

    // Backward-compatible restore for the 4SIL Paper Trader Engine.
    // Older snapshots have no field and retain the store's fresh
    // defaults (freshPaperTraderState(), set in the MemoryStore
    // constructor). ensureStateShape() fills in any missing sub-fields
    // defensively, same convention as every other engine's restore below.
    if (data.paperTrader && typeof data.paperTrader === 'object') {
      const { ensureStateShape } = require('../engines/paperTraderEngine');
      store.paperTrader = ensureStateShape(data.paperTrader);
    }

    // Backward-compatible restore for the 4SIL Next Event Intelligence
    // Engine. Older snapshots have no field and retain the store's fresh
    // defaults (freshFourSilNextEventIntelState(), set in the
    // MemoryStore constructor). ensureStateShape() fills in any missing
    // sub-fields defensively, same convention as paperTrader above.
    if (data.fourSilNextEventIntel && typeof data.fourSilNextEventIntel === 'object') {
      const { ensureStateShape } = require('../engines/fourSilNextEventIntelEngine');
      store.fourSilNextEventIntel = ensureStateShape(data.fourSilNextEventIntel);
    }
