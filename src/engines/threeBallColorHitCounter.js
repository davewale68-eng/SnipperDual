...
// lastProcessedDrawId so resolvePendingWatch() can walk them all in
// chronological order -- same fix as tierOverduePrecursorHitCounter.js.
...
function evaluateThreeBallColorHitCounter(historicalDraws, threeBallColorIntel, persistentCounter) {
  ...
  for (const draw of unprocessed) {
    resolvePendingWatch(counter.trackedColorPick, draw);
  }
