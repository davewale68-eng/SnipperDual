...
function evaluateTieNextEvent(historicalDraws, tieIntelligence, priorState) {
  ...
  if (currentGap === 0 && openForecast && openForecast.status === 'OPEN') {
  ...
  const evaluation = {
    ...
    avgActualLatency: resolved.length > 0
      ? Math.round(mean(resolved.map(p => p.actualDrawsToEvent)) * 10) / 10
      : null
  };
