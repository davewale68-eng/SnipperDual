 ...
 * ---------------------------------------------------------------------
 * tieNextEventEngine.js's Trade Penetration state machine has no
 * MONITOR/PREPARE concept the way 4SIL's badge does -- it only ever has
 * a call open or not. So the signal collapses to two states instead of
 * four:
 *   no open call (penetration.displayState WAIT | AVOID)      -> 'PAUSE'
 *   a call was live for this draw (displayState ENTER | HOLD |
 *     TAKE PROFIT | WINDOW CLOSED -- the latter two both fire on the
 *     exact draw a live call resolves, so that draw's own outcome must
 *     still be traded)                                         -> 'ENTER'
 ...
 * exactly: ladder table, starting capital, window size, config/reset API
 * shape)
 * ---------------------------------------------------------------------
 *   1. DEFAULT_PAYOUT_MULTIPLIER = 4   (MEDIUM_GATE_PRO uses 12)
 *   2. ATTEMPTS_PER_LADDER_STAGE = 2   (MEDIUM_GATE_PRO uses 3)
 *   3. RESET RULE: only ONE trigger -- 2 consecutive HITs at the SAME
...
// Ladder table -- identical to MEDIUM_GATE_PRO. Index 0 = Stage 1 (₦50) ...
...
const ENTER_DISPLAY_STATES = new Set(['ENTER', 'HOLD', 'TAKE PROFIT', 'WINDOW CLOSED']);
...
function deriveSignal(tieNextEvent) {
  ...
  return ENTER_DISPLAY_STATES.has(displayState) ? 'ENTER' : 'PAUSE';
}
