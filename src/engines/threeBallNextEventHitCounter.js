...
// BATCH-INGEST FIX: the collector posts its whole local batch every
// cycle (see api.js's ingest-cursor comments), so more than one new draw
// can land between two council cycles. Walking only the single latest
// draw (as this file originally did) silently skips any hit buried
// mid-batch and lets drawsElapsed drift out of sync with real draws.
// getUnprocessedDraws() collects every draw newer than
// lastProcessedDrawId so both resolvePendingWatch() and
// resolveRankWatch() can walk them all in chronological order -- same
// fix as tierOverduePrecursorHitCounter.js / threeBallColorHitCounter.js.
...
function evaluateThreeBallNextEventHitCounter(historicalDraws, threeSIL, persistentCounter) {
  ...
  // Walk every unprocessed draw chronologically (batch-ingest fix) instead
  // of only ever checking the single latest draw.
  const unprocessed = getUnprocessedDraws(historicalDraws, counter.lastProcessedDrawId);
  for (const draw of unprocessed) {
    resolvePendingWatch(counter.topColorPick, draw);
    resolveRankWatch(counter.rankWatch, draw);
  }
