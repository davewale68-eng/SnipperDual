/**
 * MAIN 3-BALL COLOR PAPER TRADER ENGINE — v1.0
 * (direct structural clone of threeSilPaperTraderEngine.js, re-pointed at
 *  threeBallColorEngine.js's "main" 3-Ball Color Hit Intelligence card
 *  instead of 3SIL's Next Event Leaderboard)
 *
 * PURPOSE
 * -------
 * A virtual/paper trading engine that consumes threeBallColorEngine.js's
 * analyzeThreeBallColor() output (the "🎯 3-BALL COLOR HIT INTELLIGENCE"
 * card -- the OTHER of this project's two 3-ball color forecasts,
 * alongside 3SIL's newer Next Event Leaderboard) and measures how a
 * fixed stake ladder would have performed staking its tracked color
 * every draw a forecast is live, against real draw outcomes.
 *
 *   LIVE DRAW DATA -> General Parliament (winningColor) ->
 *     threeBallColorEngine.js (analyzeThreeBallColor) -> MAIN 3-BALL
 *     COLOR PAPER TRADER -> VIRTUAL TRADE -> RESULT EVALUATOR ->
 ...
 * weight, or gate back into threeBallColorEngine or any other engine
 * (ABSOLUTE SEPARATION RULE) -- this is a read-only evaluation consumer,
 * following the exact same one-way boundary threeSilPaperTraderEngine.js
 * enforces against threeBallSeasonIntelligenceLab.
 *
 * EVERYTHING BELOW IS AN INTENTIONAL, LINE-FOR-LINE MIRROR OF
 * threeSilPaperTraderEngine.js -- same ladder table, same starting
 * capital, same 2-attempts-per-stage ladder repeat strategy, same window
 * size, same single reset trigger (2 consecutive HITs at the same
 * ladder stage), same 3.8x payout, same config/reset API shape. The
 * ONLY deliberate departure is which engine supplies the signal/market:
 *
 * SIGNAL MAPPING (the one deliberate departure)
 * ------------------------------------------------
 * threeBallColorEngine.js's analyzeThreeBallColor() has no
 * ENTER/HOLD/TAKE PROFIT/WINDOW CLOSED lifecycle (like the tie trader)
 * and no badge.active/topColor leaderboard gate (like the 3SIL trader)
 * -- it simply returns `available: true` with a continuously-tracked
 * `trackedColor` (the General Parliament's current winningColor)
 * whenever that color exists, exactly analogous to 3SIL's "the
 * leaderboard has a topColor every single cycle" continuity. The signal
 * therefore mirrors the 3SIL trader's own mapping one-for-one, just
 * swapping which fields it reads:
 *   not available OR no trackedColor (available === false ||
 *     !trackedColor)                                        -> 'PAUSE'
 *   available AND a trackedColor forecast exists              -> 'ENTER'
 * Trading is permitted ONLY on 'ENTER', exactly like both other traders.
 *
 * MARKET MODEL (same shape as the 3SIL trader, different source field)
 * -------------------------------------------------------------------
 * The market staked is "threeBallColorEngine's tracked color
 * (trackedColor, i.e. the General Parliament's winningColor) will be
 * this draw's threeBallColor" -- a single market per draw at the same
 * per-stage ladder amount, identical "flat stake per rung, fixed payout
 * multiplier on a HIT" mechanic both other traders use. The forecast
 * color used to grade a batch of newly-arrived draws is the SAME
 * trackedColor captured once for this council cycle's call -- identical
 * convention to both other traders, which also derive one signal per
 * call and replay it across every new draw in that cycle's batch rather
 * than re-deriving a signal per historical draw.
 *
 * PAYOUT (identical to the 3SIL trader, NOT the tie trader)
 * -----------------------------------------------------------------
 *   DEFAULT_PAYOUT_MULTIPLIER = 3.8   (same as threeSilPaperTraderEngine.js;
 *                                       the tie trader uses 4)
 * Ladder table, ATTEMPTS_PER_LADDER_STAGE (2), window size (10),
 * SAME_STAGE_CONSECUTIVE_HIT_RESET_THRESHOLD (2), and
 * DEFAULT_STARTING_CAPITAL (₦100,000) are all unchanged from both other
 * traders -- this engine and the 3SIL trader are staking two
 * independently-computed forecasts that often converge on the same
 * data, so they are given the same payout convention deliberately.
 *
 * DEBOUNCING / IDEMPOTENCY
 * -------------------------
 * Same as both other traders: a trade happens only when a NEW eligible
 * draw (drawId strictly greater than lastProcessedDrawId) arrives while
 * the signal is ENTER; lastProcessedDrawId is the dedup key.
 *
 * This engine is OBSERVATIONAL/EVALUATIVE ONLY: it must be called AFTER
 * threeBallColorEngine.js (analyzeThreeBallColor) each council cycle and
 * never before, and its own state must never be read back into
 * threeBallColorEngine or any gate/weight.
 */
'use strict';
...
// values to threeSilPaperTraderEngine.js / tiePaperTraderEngine.js, per the
// request to mirror them exactly.
// ---------------------------------------------------------------------------

// Ladder table -- identical to both other traders. Index 0 = Stage 1
// (₦50) ... index 11 = Stage 12 (₦3,000). Configurable via config.ladder.
const DEFAULT_LADDER = [50, 100, 150, 200, 300, 400, 600, 700, 1000, 1500, 2000, 3000];

// Identical to both other traders: each ladder stage is repeated 2x --
...
// Draw window clock -- identical to both other traders. The window
// boundary used to CLOSE a window and open the next one.
const WINDOW_SIZE_MIN = 10;
const WINDOW_SIZE_MAX = 11;
const DEFAULT_WINDOW_SIZE = WINDOW_SIZE_MIN;

// Reset trigger -- identical to both other traders: 2 consecutive HITs
// at the SAME ladder stage triggers an immediate ladder reset,
// regardless of stake size. No window-hit trigger (same reasoning as
// the tie trader: that trigger was calibrated against 4SIL's 12x payout
// and doesn't transfer cleanly to a lower-payout single-outcome market).
const SAME_STAGE_CONSECUTIVE_HIT_RESET_THRESHOLD = 2;

// Payout is 3.8-odds here (single tracked-color market) -- identical to
// the 3SIL trader's payout, NOT the tie trader's 4x.
const DEFAULT_PAYOUT_MULTIPLIER = 3.8;

// Paper capital -- identical to both other traders.
...
// ---------------------------------------------------------------------------
// State factory (persisted at store.threeBallColorPaperTrader) -- same shape as
...
function freshThreeBallColorPaperTraderState(config) {
...
// council.js passes store.threeBallColorPaperTrader directly and never
// reassigns it, so a copy here would silently detach the live state
// from the store.
function ensureStateShape(persistentState) {
  const fresh = freshThreeBallColorPaperTraderState();
...
function deriveSignal(threeBallColorIntel) {
  const t = threeBallColorIntel || {};
  if (t.available === false || !t.trackedColor) return 'PAUSE';
  return 'ENTER';
}

function deriveForecastColor(threeBallColorIntel) {
  const t = threeBallColorIntel || {};
  return t.trackedColor || null;
}
...
/**
 * Advance the main 3-Ball Color paper trader for the current council
 * cycle. Must be called AFTER threeBallColorEngine.js
 * (analyzeThreeBallColor) each cycle -- see header for the separation
 * rule.
 *
 * @param {Array} historicalDraws newest-first draw history (store.getRecentDraws)
 * @param {Object} threeBallColorIntel  this cycle's analyzeThreeBallColor() result
 * @param {Object} persistentState store.threeBallColorPaperTrader (mutated in place)
 * @returns {Object} read-only summary for snapshot / dashboard
 */
function advanceThreeBallColorPaperTrader(historicalDraws, threeBallColorIntel, persistentState) {
  const state = ensureStateShape(persistentState);
  const cfg = state.config;
  const draws = Array.isArray(historicalDraws) ? historicalDraws : [];
  const signal = deriveSignal(threeBallColorIntel);
  const forecastColor = deriveForecastColor(threeBallColorIntel);
  state.signal = signal;
...
function processOneDraw(state, cfg, draw, signal, forecastColor, enterRunId, drawsIntoEnterRun) {
  ...
  // Market model: stake the tracked color (threeBallColorEngine's
  // trackedColor) only. Result
  ...
  const trade = {
    ...
    stakePerTrade,
    totalStake,
    market: 'THREE_BALL_COLOR_FORECAST',
    forecastColor,
    outcome: actualColor,
    result,
    grossReturn,
...
module.exports = {
  ...
  defaultConfig,
  freshThreeBallColorPaperTraderState,
  ensureStateShape,
  deriveSignal,
  advanceThreeBallColorPaperTrader,
  buildSummary
};
